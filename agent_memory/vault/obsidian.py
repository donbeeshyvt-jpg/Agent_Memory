...
class ObsidianVaultAdapter(VaultAdapter):
    ...
    def _bootstrap_companion_defaults(self) -> None:
        ...
        companion_config_yaml = (
            ...
            "    # V3-O.13.5: AI viewer pool 模擬路徑 (allow_bot_author_ids / split_by_display_name) 已淨化,\n"
            "    # 不再支援單一 bot 帳號模擬多 viewer (對齊 V3-O.6 #5 「下次以 DC ID」拍板). 真壓測 = 多真實 bot 各自 token.\n"
            ...
            "  line:\n"
            "    enabled: true\n\n"
            "# ── LLM 路由 (V3-O.13 SUB-ONLINE: main_chat 線上 / sub_task 全線上獨立 key) ──────\n"
            "llm:\n"
            ...
            "    timeout_s: 60.0\n"
            "    max_packet_tokens: 100000                # V3-O.13.1 (2026-06-04 user 拍板): input 不主動截斷, 結構壓縮層自治 (recent_dialogue 12 turn cap + L3/L4 curator + memory_router 4-layer 硬上限). 詳見 第10次重生 補 doc §1\n"
            "    # V3-O.12 #FB-B v3: main_chat 線上 fallback\n"
            "    fallback_chain:\n"
            "      - { provider: openrouter, model: \"qwen/qwen3.6-35b-a3b\" }          # 備用\n"
            "      - { provider: openrouter, model: \"deepseek/deepseek-v4-flash:free\" } # 第3備用(免費)\n"
            "  # V3-O.13 #SUB-ONLINE: sub_task 全線上獨立 key (避免本地 ollama timeout)\n"
            "  sub_task_fallback_chain:\n"
            "    - { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash:free\" }\n"
            "    - { provider: openrouter_subtask, model: \"google/gemma-2-9b-it:free\" }\n"
            "  # 子任務 (V3-O.13 #SUB-ONLINE: 全線上 deepseek-v4-flash)\n"
            "  # V3-O.13.4 (2026-06-04 user 拍板): 全 15 個 sub_task timeout 統一 60s.\n"
            "  # Rationale: user「每環節都有意義, 不能讓他放棄」+ 資料正確 > 速度/成本.\n"
            "  # typical V4 Flash 1-3s 回, timeout 大小不影響 typical case 速度.\n"
            "  sub_tasks:\n"
            "    self_modification:        { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    owner_profile:            { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    umbrella_consolidation:   { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    emotion_appraisal:        { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    knowledge_summary:        { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    semantic_consolidation:   { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    narrative_synthesis:      { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    skill_consolidation:      { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    modifier_filter:          { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    overlay_delta:            { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    # V3-O.11 階段3 朋友卡記憶層 (反思 / 對話彙整 / 日重整 / 7天昇華)\n"
            "    viewer_reflection:        { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    friend_card_consolidation: { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    daily_refine:             { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    weekly_consolidate:       { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }\n"
            "    catchphrase_rewrite:      { provider: openrouter_subtask, model: \"deepseek/deepseek-v4-flash\", timeout_s: 60.0 }   # V3-O.13 #CR3 重複招牌句改寫\n"
            "    # V3-O.15.26: 看圖線路 vision 模型 (傳圖/伺服器貼圖 → 分析夾進收束 prompt). 改 model 存檔即生效, 不用重啟.\n"
            "    # 純文字模型 (如 qwen3.6-35b-a3b) 不支援視覺會 400; 要選有視覺的. 可換 google/gemini-2.5-flash / qwen2.5-vl-72b / gpt-4o-mini.\n"
            "    image_analysis:           { provider: openrouter, model: \"qwen/qwen3.6-flash\", timeout_s: 60.0 }\n"
            "  concurrency:\n"
            "    parallel_slots: 4             # V3-O.12 #L1 (sub_task 並發 2→4)\n"
            ...
            "      max_tokens: 3000              # V3-O.15 (2026-06-05 user 拍板): 1500→3000 (×2) — main_chat output 翻倍\n"
            "    local_ollama:                  # V3-O.11+ legacy (V3-O.13 SUB-ONLINE 後不用, 保留待未來 GPU 升級切回)\n"
            "      kind: ollama\n"
            "      base_url: \"http://localhost:11434\"\n"
            "      max_tokens: 500\n"
            "    openrouter_subtask:            # V3-O.12 #L3 + V3-O.13 #SUB-ONLINE: sub_task 走的獨立 OpenRouter key\n"
            "      kind: openai_compatible\n"
            "      base_url: \"https://openrouter.ai/api/v1\"\n"
            "      api_key_env: \"OPENROUTER_API_KEY_SUBTASK\"\n"
            "      max_tokens: 3000              # V3-O.15 (2026-06-05 user 拍板): 500→3000 (×6) — 動態壓縮 + 朋友卡 5000/3000 字\n"
            "    local_gemma:                   # V3-O.10 legacy (SUB-ONLINE 後不用)\n"
            ...
            "# ── Hermes 整合 ─────────────────────────────────────────\n"
            "# V3-O.15 (2026-06-05): hermes 送來的訊息直接進 40_Knowledge_Base/42_External_Knowledge/_inbox/,\n"
            "# 不走 22-step 聊天管道, 不計親密度/情緒. inbox_ingest_daemon 每 5min 自動 LLM 摘要.\n"
            "hermes:\n"
            "  enabled: false\n"
            "  trusted_user_ids: []         # hermes bot 的 user_id (填後 hermes 訊息自動導流)\n"
            "  ingest_direct: true          # true = 直接進 40_Knowledge_Base (目前僅支援 true)\n\n"
            "# ── 技能學習 / 合併 (V3-O.15.30, user 2026-06-09 拍板) ───────\n"
            "# 控制「教學偵測 → 候選 → 升格 → 合併」整條管線節奏. 每 turn / 每 daemon tick 重讀, 改即生效.\n"
            "skill_learning:\n"
            "  evidence_threshold: 3              # 教幾次升格 (1=一次即升試; 3=歷史保守值; 風險: 1 時無概念名校正空間)\n"
            "  consolidate_interval_s: 900        # 15min 合併間隔 (秒, 最小 60)\n"
            "  weekly_consolidate_interval_s: 604800   # 7 天跨層合併 (L0+L1, 秒, 最小 3600)\n\n"
            "# ── RAG 撈廣度 + 注入長度 (V3-O.15.39, user 2026-06-10 拍板) ───────\n"
            "# 對齊 §A.33「整張塞」設計 — top_k=撈幾張, max_chars=每張注入字數 (≥500). 每 turn 重讀, 改即生效.\n"
            "# 過去 hardcode 散在 memory_router (50 step15=4000, 40 step15=2000, 50 L3=25000) 不一致, 統一這個 yaml 一個來源.\n"
            "rag_retrieval:\n"
            "  skill_top_k: 3                 # 50 層 撈幾張技能卡\n"
            "  skill_max_chars: 25000         # 50 層 每張技能卡注入字數 (§A.33 整張塞)\n"
            "  kb_top_k: 3                    # 40 層 撈幾張知識卡\n"
            "  kb_max_chars: 25000            # 40 層 每張知識卡注入字數 (KB schema v12 full_content ≤22000)\n"
            "  friend_top_k: 3                # 20 層 撈幾張朋友卡\n"
            "  friend_max_chars: 5000         # 20 層 每張朋友卡注入字數\n"
            "  daily_journal_top_k: 2         # 90 層 撈幾篇日誌\n"
            "  daily_journal_max_chars: 300   # 90 層 每篇日誌注入字數\n\n"
            "# ── Personality 模式 ────────────────────────────────────\n"
